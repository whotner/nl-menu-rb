...
local function SmoothClose(frame, dur, cb)
	...
	task.delay((dur or 0.18) + 0.01, function()
		if frame.Parent and frame:GetAttribute("PopupToken") == token then
			frame.Visible = false
			-- drop back to the parent's level so a later popup can never sit under this one
...
function Library:AddWindow(hubTitle, hubImage, gameTitle)
	...
	BorderSizePixel = 0,
	-- false so a tall popup can stick out of the window instead of being cut
...
local ClockIconBackground = ACCENT
...
function WindowObj:SetWatermark(options)
	...
	if options.accent then
		for _, icon in ipairs(wm.icons) do
			-- the clock is drawn from Frames, only image icons take a tint
			if icon ~= wm.logo and icon:IsA("ImageLabel") then icon.ImageColor3 = options.accent end
		end
		ClockIconBackground = options.accent
	end
...
UIAspectRatioConstraint.Parent = ImageLabel
	...
	do local s = Instance.new("UIStroke"); s.Color = Color3.fromRGB(60, 68, 96); s.Transparency = 0.6; s.Thickness = 1.5; s.ApplyStrokeMode = Enum.ApplyStrokeMode.Contextual; s.Parent = WindowSettingsFrame end
	...
	do local s = Instance.new("UIStroke"); s.Color = Color3.fromRGB(60, 68, 96); s.Transparency = 0.6; s.Thickness = 1.2; s.ApplyStrokeMode = Enum.ApplyStrokeMode.Contextual; s.Parent = ScaleDownBar end
	...
	do local s = Instance.new("UIStroke"); s.Color = Color3.fromRGB(60, 68, 96); s.Transparency = 0.6; s.Thickness = 1.5; s.ApplyStrokeMode = Enum.ApplyStrokeMode.Contextual; s.Parent = ScaleContainer end
	...
	do local s = Instance.new("UIStroke"); s.Color = Color3.fromRGB(60, 68, 96); s.Transparency = 0.6; s.Thickness = 1.2; s.ApplyStrokeMode = Enum.ApplyStrokeMode.Contextual; s.Parent = LangDownBar end
	...
	do local s = Instance.new("UIStroke"); s.Color = Color3.fromRGB(60, 68, 96); s.Transparency = 0.6; s.Thickness = 1.5; s.ApplyStrokeMode = Enum.ApplyStrokeMode.Contextual; s.Parent = LangContainer end
	...
	function UsageObj:AddSelection(text, options, default, callback)
		...
		do local s = Instance.new("UIStroke"); s.Color = Color3.fromRGB(60, 68, 96); s.Transparency = 0.6; s.Thickness = 1.5; s.ApplyStrokeMode = Enum.ApplyStrokeMode.Contextual; s.Parent = DropPopup end
		...
		do local s = Instance.new("UIStroke"); s.Color = Color3.fromRGB(60, 68, 96); s.Transparency = 0.6; s.Thickness = 1.5; s.ApplyStrokeMode = Enum.ApplyStrokeMode.Contextual; s.Parent = DropContainer end
...
SaveText.Parent = Save
	...
	local function ShowSavedName()
		...
		SaveText.Text = activeConfigName or "Default"
	end
...
UIAspectRatioConstraint.Parent = SaveArrow
	...
	ConfigMainFrame.Position = UDim2.new(0.012, 0, 0.055, 0)
	ConfigMainFrame.Size = UDim2.new(0.52, 0, 0.66, 0)
	...
	SearchContainer.BackgroundColor3 = Color3.fromRGB(21, 24, 36)
	...
	NameInputContainer.BackgroundColor3 = Color3.fromRGB(21, 24, 36)
	...
	RecentlyDeletedPanel.BackgroundColor3 = Color3.fromRGB(21, 24, 36)
	...
	do local s = Instance.new("UIStroke"); s.Color = Color3.fromRGB(60,68,96); s.Transparency = 0.6; s.Thickness = 1.2; s.ApplyStrokeMode = Enum.ApplyStrokeMode.Contextual; s.Parent = RecentlyDeletedPanel end
	...
	RDBackBtn.MouseButton1Click:Connect(function()
		...
		task.delay(0.22, function()
			...
			Tween(ConfigScale, {Scale = 1}, 0.24, Enum.EasingStyle.Back, Enum.EasingDirection.Out)
		end)
	end)
	...
	local function MakeConfigButton(name)
		...
		do local s = Instance.new("UIStroke"); s.Color = Color3.fromRGB(60, 68, 96); s.Transparency = 0.35; s.Thickness = 1.5; s.ApplyStrokeMode = Enum.ApplyStrokeMode.Contextual; s.Parent = SettingsFrame end
		do
			local ll = Instance.new("UIListLayout")
...
SectionLabel.TextColor3 = customization.sectionTitleColor or Color3.fromRGB(150, 158, 190)
...
SectionLabel.TextTransparency = customization.sectionTitleTransparency or 0.15
...
do
	local limit = SectionLabel:FindFirstChild("ReadableTextConstraint")
	if not limit then
		limit = Instance.new("UITextSizeConstraint")
		limit.Name = "ReadableTextConstraint"
		limit.Parent = SectionLabel
	end
	...
	limit.MaxTextSize = 14
			...
			local function MakeSettings(parentFrame, elementType, label)
				...
				local SettingsFrame = Instance.new('Frame')
				SettingsFrame.Name = "SettingsFrame"
				SettingsFrame.Position = UDim2.new(0.02, 0, 1, 4)
				SettingsFrame.Size = UDim2.new(0.96, 0, 0, 0)
				SettingsFrame.BackgroundColor3 = Color3.fromRGB(21, 24, 36)
				SettingsFrame.BackgroundTransparency = 0
				SettingsFrame.BorderSizePixel = 0
				...
				function SettingsObj:AddDropdown(text, options, default, callback)
					...
					OpenBtn.MouseButton1Click:Connect(function()
						if dropOpen then closeDropdown()
							...
							-- the popup is as wide as the row it belongs to; anchoring it to the
							-- TopBar instead pushed it off to the right of the settings panel
							local popupW = math.max(Selection.AbsoluteSize.X * 0.96, 1)
							-- rows fill DropContainer (89.8% of the popup) and carry AspectRatio 6
							local rowH = (popupW * 0.8980000019073486) / 6
							local popupH = math.clamp(rowH * #options + 10, 40, 460)
							DropPopup.Size = UDim2.new(0.96, 0, 0, popupH)
			...
			function SectionObj:AddDropdown(text, options, default, callback)
               ...
               BackgroundTransparency = 0,
				...
				local Scrolls = New("ScrollingFrame", {
					Name = "Scrolls",
					Position = UDim2.new(0.04, 0, 0, 2),
					Size = UDim2.new(0.92, 0, 1, -4),
				...
				TopBar.MouseButton1Click:Connect(function()
					if dropOpen then
						...
						ApplyZIndexLadder(DownBar, NextPopupZ())
						local h = math.clamp(#options * 23 + 8, 34, 460)
	...
	local function applyCustomization()
		...
		for _, d in ipairs(MainFrame:GetDescendants()) do
			if d:IsA("TextLabel") and d.Name == "SectionLabel" then
				d.TextScaled = false
				d.TextSize = customization.sectionTitleSize or 13
				d.TextColor3 = customization.sectionTitleColor or Color3.fromRGB(150, 158, 190)
				d.TextTransparency = customization.sectionTitleTransparency or 0.15
			end
		end
		if Watermark then
