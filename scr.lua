--[[
NEVERLOSE CS2 UI Library
Fixed & Enhanced build
- Smaller menu + manual scale
- Logo toggle
- Compact mode
- Fixed arrow on user profile
- Sections no longer overflow
- Function settings hidden behind gear icon
- Smoother animations
...
local LoadAcrylic = function()
	...
	function GuiSystem.new(frame, NoAutoBackground)
		...
		Twen:Create(Part,TweenInfo.new(1,Enum.EasingStyle.Quint,Enum.EasingDirection.In),{Transparency = 0.8}):Play()
		...
		local C4 = {
			Update = nil, Collection = SurfaceGui, Enabled = true,
			Instances = { BlockMesh=BlockMesh, Part=Part, DepthOfField=DepthOfField, SurfaceGui=SurfaceGui },
			Signal = nil, Signal2 = nil
		}
		...
		pcall(function()
			C4.Signal2 = CurrentCamera:GetPropertyChangedSignal('CFrame'):Connect(function()
				if Part and Part.Parent then Part.CFrame = CurrentCamera.CFrame end
			end)
		end)
		...
		C4.Destroy = function()
			...
			Twen:Create(Part,TweenInfo.new(1),{Transparency = 1}):Play()
			task.delay(1.1, function() DepthOfField:Destroy(); Part:Destroy() end)
		end
...
function Library:AddWindow(hubTitle, hubImage, gameTitle)
	...
	-- MAIN FRAME (with ClipsDescendants so sections don't overflow)
	...
	local UICorner_GameInfo = Instance.new('UICorner')
	UICorner_GameInfo.CornerRadius = UDim.new(1, 0)
	UICorner_GameInfo.Parent = GameInfo

	local UIAspectRatioConstraint_GI = Instance.new('UIAspectRatioConstraint')
	UIAspectRatioConstraint_GI.AspectRatio = 9
	UIAspectRatioConstraint_GI.Parent = GameInfo

	local UIScale_GI = Instance.new('UIScale')
	UIScale_GI.Scale = 0.63
	UIScale_GI.Parent = GameInfo
	...
	Instance.new('UIAspectRatioConstraint', FpsIcon)
	...
	local AR_FPS = Instance.new('UIAspectRatioConstraint', FPSText)
	AR_FPS.AspectRatio = 8
	...
	Instance.new('UIAspectRatioConstraint', SignalImage)
	...
	local AR_MS = Instance.new('UIAspectRatioConstraint', MSText)
	AR_MS.AspectRatio = 8
	...
	Instance.new('UIAspectRatioConstraint', UserIcon)
	...
	Instance.new('UIAspectRatioConstraint', NeverIcon)
	local UICorner_NeverIcon = Instance.new('UICorner', NeverIcon)
	...
	Instance.new('UIAspectRatioConstraint', Profile)
	local UICorner_Profile = Instance.new('UICorner', Profile)
	UICorner_Profile.CornerRadius = UDim.new(1, 0)

	-- FPS / MS live updates (non-blocking)
	local fpsBuffer = {}
	...
	RunService.Heartbeat:Connect(function(dt)
		...
		_lastFpsUpdate += dt
		if _lastFpsUpdate < 0.25 then return end
		_lastFpsUpdate = 0
		local avg = 0
		for _, v in ipairs(fpsBuffer) do avg += v end
		if avg > 0 and FPSText.Parent then
			FPSText.Text = math.round(#fpsBuffer / avg) .. " FPS"
		end
		if MSText.Parent then
			pcall(function()
	...
	local HubIcon = New("ImageLabel", {
		Name = "HubIcon",
		Position = UDim2.new(0.014, 0, 0.013, 0),
	...
	New("TextLabel", {
		Name = "Title",
		Position = UDim2.new(0.068, 0, 0.0087, 0),
	...
	New("TextLabel", {
		Name = "GameTitle",
		Position = UDim2.new(0.068, 0, 0.047, 0),
		Size = UDim2.new(0.1, 0, 0.017, 0),
		BackgroundTransparency = 1,
		Text = gameTitle,
		TextColor3 = Color3.fromRGB(255,255,255),
		TextScaled = true,
	...
	Info.Name = "Info"
	Info.Position = UDim2.new(0.009,0,0.907,0)
	Info.Size = UDim2.new(0.2,0,0.079,0)
	...
	UserImageEl.Size = UDim2.new(0.34,0,1,0)
	...
	User.Name = "Username"
	User.Position = UDim2.new(0.30,0,0.15,0)
	User.Size = UDim2.new(0.6,0,0.4,0)
	...
	DaysLeft.Position = UDim2.new(0.30,0,0.6,0)
	DaysLeft.Size = UDim2.new(0.6,0,0.25,0)
	...
	WindowSettings.Position = UDim2.new(0.45,0,0.35,0)
	WindowSettings.Size = UDim2.new(0.6,0,0.5,0)
	...
	IconHolder.Position = UDim2.new(0.9999997615814209,0,0.5,0)
	IconHolder.Size = UDim2.new(0, 18, 0, 18)
	IconHolder.AnchorPoint = Vector2.new(1,0.5)
	IconHolder.BackgroundTransparency = 1
	IconHolder.Parent = WindowSettings
	...
	WindowSettingsFrame.Position = UDim2.new(1.2,0,-10,0)
	WindowSettingsFrame.Size = UDim2.new(2.299,0,11,0)
	WindowSettingsFrame.BackgroundColor3 = Color3.fromRGB(17,20,30)
	WindowSettingsFrame.BackgroundTransparency = 0.3
	...
	-- Profile row inside settings
	local WSProfile = Instance.new("Frame")
	WSProfile.Name = "Profile"
	WSProfile.Size = UDim2.new(1,0,0.2,0)
	...
	do
		...
		un.Name = "Username"
		un.Position = UDim2.new(0.3,0,0.008,0)
		un.Size = UDim2.new(0.5,0,0.8,0)
	...
	do
		...
		dl.Name = "DaysLeft"
		dl.Position = UDim2.new(0.3,0,0.5,0)
		dl.Size = UDim2.new(0.5,0,0.34,0)
		dl.BackgroundTransparency = 1
		dl.Text = daysLeft
		dl.TextColor3 = Color3.fromRGB(255,255,255)
		dl.TextScaled = true
		dl.Font = Enum.Font.SourceSansBold
		dl.TextTransparency = 0.6
		dl.ZIndex = 130
	...
	-- ============================================================
	-- SETTINGS CONTAINER (colorpicker / scale / language)
	-- ============================================================
	local SettingsContainer = Instance.new("Frame")
	SettingsContainer.Name = "SettingsContainer"
	SettingsContainer.Position = UDim2.new(0,0,0.23,0)
	...
	WSColorpicker.Size = UDim2.new(1,0,0.2,0)
	...
	wsColorBtn.Position = UDim2.new(0.875,0,0.57,0)
	wsColorBtn.Size = UDim2.new(0.08,0,0.5,0)
	...
	wsColorFrame.Position = UDim2.new(1.1,0,0,0)
	...
	do local s = Instance.new("UIStroke"); s.Color = Color3.fromRGB(255,255,255); s.Transparency = 0.92; s.Parent = wsColorFrame end
	do local a = Instance.new("UIAspectRatioConstraint"); a.AspectRatio = 1.1; a.Parent = wsColorFrame end

	local wsRGB = Instance.new("ImageButton")
	wsRGB.Name = "RGB"
	wsRGB.Position = UDim2.new(0.067,0,0.068,0)
	wsRGB.Size = UDim2.new(0.74,0,0.74,0)
	...
	wsDark.Position = UDim2.new(0.831940293,0,0.068,0)
	wsDark.Size = UDim2.new(0.14,0,0.74,0)
	...
	wsDarkCircle.Size = UDim2.new(1.4,0,0,5)
	...
	wsHex.Name = "colorHex"
	wsHex.Position = UDim2.new(0.0717,0,0.855,0)
	wsHex.Size = UDim2.new(0.44,0,0.09,0)
	...
	wsCopy.Name = "Copy"
	wsCopy.Position = UDim2.new(0.54,0,0.855,0)
	wsCopy.Size = UDim2.new(0.39,0,0.09,0)
	...
	ScaleArrow.Position = UDim2.new(0.87,0,0.1,0)
	ScaleArrow.Size = UDim2.new(0.1,0,0.8,0)
	ScaleArrow.BackgroundTransparency = 1
	ScaleArrow.Image = "rbxassetid://10709790948"
	ScaleArrow.ImageTransparency = 0.4
	...
	ScaleDefault.Position = UDim2.new(0.654,0,0.17,0)
	ScaleDefault.Size = UDim2.new(0.23,0,0.6,0)
	...
	ScaleDefault.TextTransparency = 0.7
	...
	ScaleDownBar.Size = UDim2.new(0.59,0,0,0)
	...
	do local s = Instance.new("UIStroke"); s.Color = Color3.fromRGB(28,32,48); s.Transparency = 0.8; s.ApplyStrokeMode = Enum.ApplyStrokeMode.Border; s.Parent = ScaleDownBar end
	do
	...
	do local ll = Instance.new("UIListLayout"); ll.SortOrder = Enum.SortOrder.LayoutOrder; ll.Padding = UDim.new(0,2); ll.Parent = ScaleContainer end
	...
	LangArrow.Position = UDim2.new(0.87,0,0.1,0)
	LangArrow.Size = UDim2.new(0.1,0,0.8,0)
	LangArrow.BackgroundTransparency = 1
	LangArrow.Image = "rbxassetid://10709790948"
	LangArrow.ImageTransparency = 0.4
	...
	LangDefault.Position = UDim2.new(0.654,0,0.17,0)
	LangDefault.Size = UDim2.new(0.23,0,0.6,0)
	...
	LangDefault.TextTransparency = 0.7
	...
	LangDownBar.Size = UDim2.new(0.59,0,0,0)
	...
	do local s = Instance.new("UIStroke"); s.Color = Color3.fromRGB(28,32,48); s.Transparency = 0.8; s.ApplyStrokeMode = Enum.ApplyStrokeMode.Border; s.Parent = LangDownBar end
	do
		local ds = Instance.new("ImageLabel"); ds.Name = "DropShadow"
		ds.Position = UDim2.new(0.5,0,0.5,0); ds.Size = UDim2.new(1,47,1,47)
		ds.AnchorPoint = Vector2.new(0.5,0.5); ds.BackgroundTransparency = 1; ds.BorderSizePixel = 0
	...
	do local ll = Instance.new("UIListLayout"); ll.SortOrder = Enum.SortOrder.LayoutOrder; ll.Padding = UDim.new(0,2); ll.Parent = LangContainer end
	...
	do
		...
		-- Scale dropdown
		local scales = {"Auto", "120%", "110%", "100%", "90%", "80%", "70%", "60%", "50%"}
		local scaleVals = {1, 1.2, 1.1, 1.0, 0.9, 0.8, 0.7, 0.6, 0.5}
		...
		WindowSettings.MouseButton1Click:Connect(function()
			wsOpen = not wsOpen
			if wsOpen then
				SmoothOpen(WindowSettingsFrame, 0.3, 0.2)
			else
				closeWS()
			end
		end)
	...
	-- ============================================================
	-- USAGE CONTAINER (settings shortcuts added by user)
	-- ============================================================
	local UsageContainer = Instance.new("Frame")
	UsageContainer.Name = "UsageContainer"
	UsageContainer.Position = UDim2.new(0,0,0.67,0)
	...
	local Tab = New("ScrollingFrame", {
		Name = "Tab",
		Position = UDim2.new(0.008, 0, 0.085, 0),
		Size = UDim2.new(0.2, 0, 0.815, 0),
		BackgroundTransparency = 1,
		BorderSizePixel = 0,
	...
	local SearchBtn = New("ImageButton", {
		Name = "Search",
		Position = UDim2.new(0.938, 0, 0.2, 0),
		Size = UDim2.new(0.034, 0, 0.45, 0),
		BackgroundTransparency = 1,
		Image = "http://www.roblox.com/asset/?id=6031154871",
		ImageTransparency = 0.2,
	}, Header)
	Instance.new("UIAspectRatioConstraint", SearchBtn)

	-- Search box (single instance)
	...
	SaveIcon.Position = UDim2.new(0.02,0,0.2,0)
	SaveIcon.Size = UDim2.new(0.27,0,0.7,0)
	SaveIcon.BackgroundTransparency = 1
	SaveIcon.Image = "http://www.roblox.com/asset/?id=6035067857"
	SaveIcon.ImageTransparency = 0.2
	SaveIcon.Parent = Save
	Instance.new("UIAspectRatioConstraint", SaveIcon)

	local SaveText = Instance.new('TextLabel')
	SaveText.Name = "SaveText"
	SaveText.Position = UDim2.new(0.23,0,0.19,0)
	SaveText.Size = UDim2.new(0.55,0,0.7,0)
	...
	SaveArrow.Position = UDim2.new(0.8,0,0.25,0)
	SaveArrow.Size = UDim2.new(0.2,0,1,0)
	...
	local AR_SaveArrow = Instance.new('UIAspectRatioConstraint', SaveArrow)
	AR_SaveArrow.AspectRatio = 2
	AR_SaveArrow.AspectType = Enum.AspectType.ScaleWithParentSize
	...
	do local s = Instance.new("UIStroke"); s.Color = Color3.fromRGB(28,32,48); s.Transparency = 0.8; s.ApplyStrokeMode = Enum.ApplyStrokeMode.Border; s.Parent = ConfigMainFrame end
	...
	Instance.new("UIAspectRatioConstraint", CloudIcon)
	...
	Instance.new("UIAspectRatioConstraint", AutoSaveBtn).AspectRatio = 1.2
	...
	Instance.new("UIAspectRatioConstraint", RecentlyDeletedBtn)
	...
	Instance.new("UIAspectRatioConstraint", InsertConfigBtn)
	...
	Instance.new("UIAspectRatioConstraint", SearchIconImg)

	local SearchBoxCfg = Instance.new("TextBox")
	SearchBoxCfg.Name = "SearchBox"
	SearchBoxCfg.Position = UDim2.new(0.12, 0, 0, 0)
	SearchBoxCfg.Size = UDim2.new(0.85, 0, 1, 0)
	SearchBoxCfg.BackgroundTransparency = 1
	SearchBoxCfg.Text = ""
	SearchBoxCfg.PlaceholderText = "Search"
	SearchBoxCfg.PlaceholderColor3 = Color3.fromRGB(200, 200, 210)
	SearchBoxCfg.TextColor3 = Color3.fromRGB(210, 215, 225)
	SearchBoxCfg.TextSize = 16
	SearchBoxCfg.Font = Enum.Font.Gotham
	SearchBoxCfg.ZIndex = 1000
	SearchBoxCfg.TextXAlignment = Enum.TextXAlignment.Left
	SearchBoxCfg.ClearTextOnFocus = false
	SearchBoxCfg.Parent = SearchContainer
	...
	local function MakeConfigButton(name)
		...
		Instance.new("UIAspectRatioConstraint", LoadConfigBtn)
		...
		do local ll = Instance.new("UIListLayout"); ll.Padding = UDim.new(0, 2); ll.Parent = SettingsFrame end
	...
	SearchBoxCfg:GetPropertyChangedSignal("Text"):Connect(function()
		local query = SearchBoxCfg.Text:lower()
		for _, c in ipairs(ListContainer:GetChildren()) do
	...
	AutoSaveBtn.MouseButton1Click:Connect(function()
		...
		if autoSaveEnabled then
			...
			if activeRow then SaveNamedConfig(SaveText.Text) end
		else
	...
	task.spawn(function()
		while _alive do
			task.wait(30)
			if _alive and autoSaveEnabled and activeRow then
				local n = SaveText.Text
				if n ~= "Save" and n ~= "" then SaveNamedConfig(n) end
			end
		end
	end)
	...
	TriggerSaveConfig.MouseButton1Click:Connect(function()
		...
		if configOpen then
			...
			ConfigMainFrame.BackgroundTransparency = 1
			ConfigMainFrame.Visible = true
			Tween(ConfigMainFrame, {BackgroundTransparency = 0.01}, 0.25)
		else
			closeConfigPanel()
		end
	end)
	...
	SearchBtn.MouseButton1Click:Connect(function()
		...
		if searchOpen then
			...
			SearchBox.Size = UDim2.new(0, 0, 0.6, 0)
			Tween(SearchBox, {Size = UDim2.new(0.4, 0, 0.6, 0)}, 0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out)
			task.wait(0.3)
			SearchBox:CaptureFocus()
		else
			Tween(SearchBox, {Size = UDim2.new(0, 0, 0.6, 0)}, 0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.In)
			task.wait(0.21)
	...
	-- ============================================================
	-- TabHose (content columns) with ClipsDescendants
	-- ============================================================
	local TabHose = New("Frame", {
		Name = "TabHose",
		Position = UDim2.new(0.01, 0, 0.092, 0),
		Size = UDim2.new(0.99, 0, 0.896, 0),
		BackgroundTransparency = 1,
		ClipsDescendants = true,
	}, Frame2)
	New("UIListLayout", {
	...
	ApplySearch = function(query)
		...
		for _, child in ipairs(TabHose:GetChildren()) do
			if not (child:IsA("Frame") and (child.Name == "Left" or child.Name == "Right"
				or child.Name:sub(1,3) == "STL" or child.Name:sub(1,3) == "STR")) then goto skip end
			for _, section in ipairs(child:GetChildren()) do
				if not section:IsA("Frame") or section.Name ~= "Section" then goto cont end
				local elementsFrame = section:FindFirstChild("Elements")
				if not elementsFrame then goto cont end
				local anyVisible = false
				for _, elem in ipairs(elementsFrame:GetChildren()) do
					if elem:IsA("UIListLayout") or elem:IsA("UICorner") or elem:IsA("UIStroke")
						or elem:IsA("UIPadding") or elem:IsA("UIAspectRatioConstraint") then goto cont2 end
					if not elem:IsA("GuiObject") then goto cont2 end
					local labelEl = elem:FindFirstChild("TextToggle") or elem:FindFirstChild("TextDropdown") or elem:FindFirstChild("Text")
					if not labelEl then
						elem.Visible = true; anyVisible = true; goto cont2
					end
	...
	Line3.Name = "Line3"
	Line3.Position = UDim2.new(-0.01,0,0,0)
	...
	Line9.Name = "Line9"
	Line9.Position = UDim2.new(0.229,0,0.08,0)
	Line9.Size = UDim2.new(0.76,0,0,1)
	Line9.BackgroundColor3 = Color3.fromRGB(162,162,162)
	Line9.BackgroundTransparency = 0.8
	Line9.BorderSizePixel = 0
	Line9.Parent = MainFrame

	local Line1 = Instance.new('Frame')
	Line1.Name = "Line1"
	Line1.Position = UDim2.new(0.01,0,0.08,0)
	Line1.Size = UDim2.new(0.209,0,0,1)
	...
	Line4.Name = "Line4"
	Line4.Position = UDim2.new(0.008,0,0.9,0)
	Line4.Size = UDim2.new(0.209,0,0,1)
	...
	ToggleShadow.ImageColor3 = Color3.fromRGB(12,14,22)
	ToggleShadow.ImageTransparency = 0
	...
	local function toggleGui()
		...
		if guiOpen then
			...
			if AcrylicBlur.Signal then AcrylicBlur.Signal:Disconnect() end
			AcrylicBlur.Signal = RunService.RenderStepped:Connect(AcrylicBlur.Update)
	...
	UserInputService.InputBegan:Connect(function(input, gp)
		if gp then return end
		if input.KeyCode == Enum.KeyCode.H then toggleGui() end
	end)
	...
	function WindowObj:SetScale(value)
		value = math.clamp(value or 1, 0.4, 1.6)
		UIScale.Scale = value
		if ScaleDefault then ScaleDefault.Text = string.format("%d%%", math.floor(value*100)) end
	end

	function WindowObj:SetLogoEnabled(enabled)
		if HubIcon then HubIcon.Visible = enabled ~= false end
	end

	function WindowObj:SetCompact(enabled)
		Aspect.AspectRatio = enabled and 1.15 or 1.4
	end

	function WindowObj:SetBackgroundTransparency(value)
		MainFrame.BackgroundTransparency = math.clamp(value or 0.2, 0, 1)
	end

	function WindowObj:GetFrame()
		return MainFrame
	end
	...
	-- ADD TAB
	-- ============================================================
	function WindowObj:AddTab(name, icon, showText)
		tabOrder = tabOrder + 1
		...
		local TabIcon = New("ImageLabel", {
			Name = "TabIcon",
			Position = showText == false and UDim2.new(0.5,0,0.5,0) or UDim2.new(0.1,0,0.5,0),
			AnchorPoint = Vector2.new(0.5, 0.5),
			Size = UDim2.new(showText == false and 0.45 or 0.2, 0, 0.65, 0),
			BackgroundTransparency = 1,
			Image = "",
			ImageColor3 = isFirst and mainColor or Color3.fromRGB(255, 255, 255),
			ImageTransparency = isFirst and 0 or 0.6,
		}, TabButtonFrame)
		if icon and icon ~= "" then GetImageData(icon, TabIcon) end
		table.insert(_coloredElements, { type = "tabicon", ref = TabIcon })
		New("UIAspectRatioConstraint", { AspectRatio = 1 }, TabIcon)

		local TabText = New("TextLabel", {
			Name = "TabText",
			Position = UDim2.new(0.23, 0, 0.235, 0),
			Size = UDim2.new(0.707, 0, 0.51, 0),
			BackgroundTransparency = 1,
			Text = showText == false and "" or name,
			TextColor3 = Color3.fromRGB(255, 255, 255),
			TextScaled = true,
			Font = Enum.Font.SourceSansSemibold,
			TextTransparency = isFirst and 0 or 0.6,
			TextXAlignment = Enum.TextXAlignment.Left,
		...
		local function ShowContent()
			...
			Left.Visible = true
			Right.Visible = true
			Tween(Left, { Position = UDim2.new(0, 0, 0, 0) }, 0.25)
			Tween(Right, { Position = UDim2.new(0, 0, 0, 0) }, 0.25)
		end
		...
		local function ActivateTab()
			for _, t in ipairs(tabs) do
				...
				Tween(t.txt, { TextTransparency = 0.6 }, 0.2)
				t.hide()
			end
		...
		function TabObj:AddSection(title, side)
			...
			SexstionLabel.Size = UDim2.new(0.5,0,0.06,0)
			SexstionLabel.BackgroundTransparency = 1
			SexstionLabel.Parent = Section
			do
				local a = Instance.new('UIAspectRatioConstraint', SexstionLabel)
				a.AspectRatio = 11
			...
			SectionLabel.Position = UDim2.new(0.07,0,0,0)
			...
			do
				local a = Instance.new('UIAspectRatioConstraint', SectionLabel)
				a.AspectRatio = 13
				a.AspectType = Enum.AspectType.ScaleWithParentSize
			end
			...
			-- ============================================================
			-- MakeSettings: gear button to hide function settings
			-- ============================================================
			local function MakeSettings(parentFrame, elementType, label)
				...
				New("UIAspectRatioConstraint", { AspectRatio = 1 }, SettingsBtn)

				SettingsBtn.MouseEnter:Connect(function()
					Tween(SettingsBtn, { ImageColor3 = mainColor, ImageTransparency = 0 }, 0.15)
				end)
				SettingsBtn.MouseLeave:Connect(function()
					Tween(SettingsBtn, { ImageColor3 = Color3.fromRGB(180, 180, 200), ImageTransparency = 0.35 }, 0.15)
				end)
				...
				do local c = Instance.new('UICorner'); c.Parent = SettingsFrame end
				do local s = Instance.new('UIStroke'); s.Color = Color3.fromRGB(40,44,65); s.Transparency = 0.9; s.Parent = SettingsFrame end

				local SFLabelContainer = Instance.new('TextLabel')
				SFLabelContainer.Name = "LabelContainer"
				SFLabelContainer.Position = UDim2.new(0.05, 0, 0, 0)
				SFLabelContainer.Size = UDim2.new(1, 0, 0.3, 0)
				...
				do
					local a = Instance.new('UIAspectRatioConstraint', SFLabelContainer)
					a.AspectRatio = 14
					a.AspectType = Enum.AspectType.ScaleWithParentSize
				end
				...
				SFContainer.Position = UDim2.new(0.05, 0, 1, 0)
				SFContainer.Size = UDim2.new(0.9, 0, 0.97, 20)
				...
				New('UIStroke', { Color = Color3.fromRGB(255,255,255), Transparency = 0.9, ApplyStrokeMode = Enum.ApplyStrokeMode.Border }, SFContainer)
				...
				local function closeSF()
					settingsOpen = false
					Tween(SettingsFrame, { Size = UDim2.new(0.898, 0, 0, 0), BackgroundTransparency = 1 }, 0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.In)
				...
				SettingsBtn.MouseButton1Click:Connect(function()
					...
					if settingsOpen then
						SettingsFrame.Visible = true
						SettingsFrame.BackgroundTransparency = 1
						SettingsFrame.Size = UDim2.new(0.898, 0, 0, 0)
						task.defer(function()
							Tween(SettingsFrame, { BackgroundTransparency = 0.02 }, 0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out)
						end)
					else
						closeSF()
					end
				...
				function SettingsObj:AddToggle(text, default, callback)
					...
					New("UIAspectRatioConstraint", { AspectRatio = 7.5, AspectType = Enum.AspectType.ScaleWithParentSize }, Toggle)
					New("Frame", { Name = "Lines", Position = UDim2.new(0.05, 0, 1, 0), Size = UDim2.new(0.9, 0, 0, 1), BackgroundColor3 = Color3.fromRGB(162, 162, 162), BackgroundTransparency = 0.9, ZIndex = 1000, BorderSizePixel = 0 }, Toggle)
					New("TextLabel", { Name = "TextToggle", Position = UDim2.new(0.048, 0, 0.3, 0), Size = UDim2.new(0.66, 0, 0.6, 0), BackgroundTransparency = 1, Text = text, TextColor3 = Color3.fromRGB(255, 255, 255), TextScaled = true, Font = Enum.Font.SourceSansSemibold, TextTransparency = 0.1, TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 1001 }, Toggle)
					local Effect = New("Frame", { Name = "Effect", Position = UDim2.new(0.84, 0, 0.3, 0), Size = UDim2.new(0.11, 0, 0.56, 0), BackgroundColor3 = enabled and mainColor or Color3.fromRGB(0, 0, 0), ZIndex = 1001 }, Toggle)
					New("UICorner", { CornerRadius = UDim.new(0.6, 0) }, Effect)
					New("UIStroke", { Transparency = 0.8, Thickness = 0.8 }, Effect)
					local Icon = New("Frame", { Name = "Icon", Position = enabled and UDim2.new(0.41, 0, 0.045, 0) or UDim2.new(0.08, 0, 0.045, 0), Size = UDim2.new(1, 0, 0.9, 0), BackgroundColor3 = Color3.fromRGB(255, 255, 255), BackgroundTransparency = enabled and 0 or 0.5, ZIndex = 1002 }, Effect)
					New("UICorner", { CornerRadius = UDim.new(1, 0) }, Icon)
					New("UIAspectRatioConstraint", {}, Icon)
					local Btn = New("TextButton", { Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 1, Text = "", ZIndex = 1010 }, Toggle)
					local function Set(val)
						enabled = val
						Tween(Effect, { BackgroundColor3 = enabled and mainColor or Color3.fromRGB(0, 0, 0) }, 0.25)
						Tween(Icon, { Position = enabled and UDim2.new(0.41, 0, 0.045, 0) or UDim2.new(0.08, 0, 0.045, 0), BackgroundTransparency = enabled and 0 or 0.5 }, 0.25)
				...
				function SettingsObj:AddSlider(text, min, max, default, callback, suffix)
					...
					local dragging = false
					local Slider = New("Frame", { Name = "Slider", Size = UDim2.new(1, 0, 0.2, 0), BackgroundTransparency = 1, LayoutOrder = settingsElemCount, ZIndex = 1000 }, SFContainer)
					New("UIAspectRatioConstraint", { AspectRatio = 7.5, AspectType = Enum.AspectType.ScaleWithParentSize }, Slider)
					New("Frame", { Name = "Lines", Position = UDim2.new(0.05, 0, 1, 0), Size = UDim2.new(0.9, 0, 0, 1), BackgroundColor3 = Color3.fromRGB(162, 162, 162), BackgroundTransparency = 0.9, ZIndex = 1002, BorderSizePixel = 0 }, Slider)
					New("TextLabel", { Name = "TextToggle", Position = UDim2.new(0.048, 0, 0.3, 0), Size = UDim2.new(0.66, 0, 0.6, 0), BackgroundTransparency = 1, Text = text, TextColor3 = Color3.fromRGB(255, 255, 255), TextScaled = true, Font = Enum.Font.SourceSansSemibold, TextTransparency = 0.1, TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 1001 }, Slider)
					local Effect = New("Frame", { Name = "Effect", Position = UDim2.new(0.498, 0, 0.25, 0), Size = UDim2.new(0.5, 0, 0.7, 0), BackgroundTransparency = 1, ZIndex = 1001 }, Slider)
					local SliderValue = New("TextLabel", { Name = "SliderValue", Position = UDim2.new(0.699, 0, 0.1, 0), Size = UDim2.new(0.25, 0, 0.81, 0), BackgroundColor3 = Color3.fromRGB(33,37,53), Text = tostring(value) .. suffix, TextColor3 = Color3.fromRGB(255, 255, 255), TextScaled = true, Font = Enum.Font.SourceSansSemibold, TextTransparency = 0.2, ZIndex = 1002 }, Effect)
					New("UICorner", { CornerRadius = UDim.new(0, 5) }, SliderValue)
					local Line = New("Frame", { Name = "Line", Position = UDim2.new(0.04, 0, 0.32, 0), Size = UDim2.new(0.6, 0, 0.24, 0), BackgroundColor3 = Color3.fromRGB(33,37,53), ZIndex = 1001 }, Effect)
					New("UICorner", { CornerRadius = UDim.new(1, 0) }, Line)
					local InLine = New("Frame", { Name = "InLine", Size = UDim2.new(0, 0, 1, 0), BackgroundColor3 = mainColor, ZIndex = 1002 }, Line)
				...
				function SettingsObj:AddDropdown(text, options, default, callback)
					...
					local dropOpen = false
					local selectedBtns = {}
					local Selection = New("Frame", { Name = "Selection", Size = UDim2.new(1, 0, 0, 28), BackgroundTransparency = 1, LayoutOrder = settingsElemCount, ClipsDescendants = false, ZIndex = 1000 }, SFContainer)
					New("UIAspectRatioConstraint", { AspectRatio = 7.5, AspectType = Enum.AspectType.ScaleWithParentSize }, Selection)
					New("Frame", { Name = "Lines", Position = UDim2.new(0.05, 0, 1, 0), Size = UDim2.new(0.9, 0, 0, 1), BackgroundColor3 = Color3.fromRGB(162, 162, 162), BackgroundTransparency = 0.9, BorderSizePixel = 0, ZIndex = 1000 }, Selection)
					New("TextLabel", { Name = "TextToggle", Position = UDim2.new(0.048, 0, 0.3, 0), Size = UDim2.new(0.6, 0, 0.6, 0), BackgroundTransparency = 1, Text = text, TextColor3 = Color3.fromRGB(255, 255, 255), TextScaled = true, Font = Enum.Font.SourceSansSemibold, TextTransparency = 0.1, TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 1001 }, Selection)
					local SelArrow = New("ImageLabel", { Name = "TextArrow", Position = UDim2.new(0.87, 0, 0.15, 0), Size = UDim2.new(0.1, 0, 0.8, 0), BackgroundTransparency = 1, Image = "rbxassetid://10709790948", ImageColor3 = Color3.fromRGB(255, 255, 255), ImageTransparency = 0.1, ScaleType = Enum.ScaleType.Fit, Rotation = -90, ZIndex = 1000 }, Selection)
					local TextDefault = New("TextLabel", { Name = "TextDefault", Position = UDim2.new(0.64, 0, 0.25, 0), Size = UDim2.new(0.23, 0, 0.6, 0), BackgroundTransparency = 1, Text = options[1] or "Auto", TextColor3 = Color3.fromRGB(255, 255, 255), TextScaled = true, Font = Enum.Font.SourceSansSemibold, TextTransparency = 0.5, ZIndex = 1001, TextXAlignment = Enum.TextXAlignment.Right }, Selection)
					local DropPopup = New("Frame", { Name = "Dropdown", Position = UDim2.new(1, 0, 0, 0), Size = UDim2.new(0.69, 0, 0, 100), BackgroundColor3 = Color3.fromRGB(16,19,28), BackgroundTransparency = 1, BorderSizePixel = 0, Visible = false, ZIndex = 1000 }, Selection)
					New("UICorner", { CornerRadius = UDim.new(0, 5) }, DropPopup)
					New("UIStroke", { Color = Color3.fromRGB(28,32,48), Transparency = 0.8, ApplyStrokeMode = Enum.ApplyStrokeMode.Border }, DropPopup)
					local DropContainer = New("Frame", { Name = "Container", Position = UDim2.new(0.05, 0, 0.1, 0), Size = UDim2.new(0.898, 0, 0.8, 0), BackgroundTransparency = 1, ZIndex = 1000, AutomaticSize = Enum.AutomaticSize.Y }, DropPopup)
					...
					for i, opt in ipairs(options) do
						local BtnRow = New("Frame", { Name = "Buttons", Size = UDim2.new(1, 0, 0.2, 0), BackgroundTransparency = 1, ZIndex = 1001, LayoutOrder = i }, DropContainer)
						New("UIAspectRatioConstraint", { AspectRatio = 6, AspectType = Enum.AspectType.ScaleWithParentSize }, BtnRow)
						New("TextLabel", { Name = "Label", Position = UDim2.new(0, 0, 0.2, 0), Size = UDim2.new(1, 0, 0.7, 0), BackgroundTransparency = 1, Text = opt, TextColor3 = Color3.fromRGB(255, 255, 255), TextScaled = true, Font = Enum.Font.SourceSansSemibold, TextTransparency = 0.1, ZIndex = 1002, TextXAlignment = Enum.TextXAlignment.Left }, BtnRow)
						local SelBtn = New("TextButton", { Name = "Selected", Position = UDim2.new(0.8, 0, 0, 0), Size = UDim2.new(0.2, 0, 0.8, 0), BackgroundTransparency = 1, BorderSizePixel = 0, Text = "", TextColor3 = mainColor, TextScaled = true, ZIndex = 1004 }, BtnRow)
						New("UICorner", { CornerRadius = UDim.new(0, 5) }, SelBtn)
						selectedBtns[opt] = SelBtn
						local RowBtn = New("TextButton", { Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 1, Text = "", ZIndex = 1005 }, BtnRow)
					...
					local OpenBtn = New("TextButton", { Name = "OpenBtn", Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 1, Text = "", ZIndex = 1500 }, Selection)
					OpenBtn.MouseButton1Click:Connect(function()
						if dropOpen then closeDropdown()
						else
							CloseAllPopupsExcept(DropPopup)
				...
				function SettingsObj:AddColorpicker(text, defaultColor, callback)
					...
					local Colorpicker = New("Frame", { Name = "Colorpicker", Size = UDim2.new(1, 0, 0, 28), BackgroundTransparency = 1, BorderSizePixel = 0, LayoutOrder = settingsElemCount, ClipsDescendants = false, ZIndex = 1000 }, SFContainer)
					New("UIAspectRatioConstraint", { AspectRatio = 7.5, AspectType = Enum.AspectType.ScaleWithParentSize }, Colorpicker)
					New("Frame", { Name = "Lines", Position = UDim2.new(0.05, 0, 1, 0), Size = UDim2.new(0.89, 0, 0, 1), BackgroundColor3 = Color3.fromRGB(162, 162, 162), BackgroundTransparency = 0.9, BorderSizePixel = 0, ZIndex = 1001 }, Colorpicker)
					New("TextLabel", { Position = UDim2.new(0.048, 0, 0.3, 0), Size = UDim2.new(0.66, 0, 0.6, 0), BackgroundTransparency = 1, Text = text, TextColor3 = Color3.fromRGB(255, 255, 255), TextScaled = true, Font = Enum.Font.SourceSansBold, TextTransparency = 0.1, TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 1001 }, Colorpicker)
					local colorpickerButton = New("ImageButton", { Name = "colorpickerButton", AnchorPoint = Vector2.new(0.5, 0.5), Position = UDim2.new(0.875, 0, 0.57, 0), Size = UDim2.new(0.08, 0, 0.5, 0), BackgroundColor3 = defaultColor or Color3.fromRGB(255, 255, 255), BorderSizePixel = 0, Image = "", ZIndex = 1010 }, Colorpicker)
					New("UICorner", { CornerRadius = UDim.new(0, 6) }, colorpickerButton)
					local colorpickerFrame = New("Frame", { Name = "colorpickerFrame", Position = UDim2.new(1.1, 0, 0, 0), Size = UDim2.new(1, 0, 7, 0), BackgroundColor3 = Color3.fromRGB(15,17,26), BorderSizePixel = 0, Visible = false, ZIndex = 1350, ClipsDescendants = false }, Colorpicker)
					...
					local RGB = New("ImageButton", { Name = "RGB", BackgroundTransparency = 1, BorderSizePixel = 0, Position = UDim2.new(0.067, 0, 0.068, 0), Size = UDim2.new(0.74, 0, 0.74, 0), AutoButtonColor = false, Image = "rbxassetid://6523286724", ZIndex = 1360 }, colorpickerFrame)
					local RGBCircle = New("ImageLabel", { Name = "RGBCircle", BackgroundTransparency = 1, BorderSizePixel = 0, Size = UDim2.new(0, 14, 0, 14), Image = "rbxassetid://3926309567", ImageRectOffset = Vector2.new(628, 420), ImageRectSize = Vector2.new(48, 48), ZIndex = 1361 }, RGB)
					local Darkness = New("ImageButton", { Name = "Darkness", BackgroundColor3 = Color3.fromRGB(255, 255, 255), BorderSizePixel = 0, Position = UDim2.new(0.832, 0, 0.068, 0), Size = UDim2.new(0.14, 0, 0.74, 0), AutoButtonColor = false, Image = "rbxassetid://156579757", ZIndex = 1360 }, colorpickerFrame)
					local DarknessCircle = New("Frame", { Name = "DarknessCircle", AnchorPoint = Vector2.new(0.5, 0.5), BackgroundColor3 = Color3.fromRGB(255, 255, 255), BorderSizePixel = 0, Position = UDim2.new(0.5, 0, 0, 0), Size = UDim2.new(1.4, 0, 0, 5), ZIndex = 1361 }, Darkness)
					New("UICorner", { CornerRadius = UDim.new(1, 0) }, DarknessCircle)
					local colorHex = New("TextLabel", { BackgroundColor3 = Color3.fromRGB(15,17,26), Position = UDim2.new(0.0717, 0, 0.855, 0), Size = UDim2.new(0.44, 0, 0.09, 0), Font = Enum.Font.ArialBold, Text = "#FFFFFF", TextColor3 = Color3.fromRGB(210, 215, 225), TextScaled = true, ZIndex = 1360 }, colorpickerFrame)
					New("UICorner", { CornerRadius = UDim.new(0, 4) }, colorHex)
					local Copy = New("TextButton", { BackgroundColor3 = Color3.fromRGB(15,17,26), Position = UDim2.new(0.54, 0, 0.855, 0), Size = UDim2.new(0.39, 0, 0.09, 0), AutoButtonColor = false, Font = Enum.Font.ArialBold, Text = "Copy", TextColor3 = Color3.fromRGB(210, 215, 225), TextScaled = true, ZIndex = 1360 }, colorpickerFrame)
					...
					UserInputService.InputEnded:Connect(function(input)
						if input.UserInputType == Enum.UserInputType.MouseButton1 then
							WheelDown = false; SlideDown = false
						end
					end)
					UserInputService.InputChanged:Connect(function(input)
						if input.UserInputType == Enum.UserInputType.MouseMovement then
					...
					local function setcolor(tbl)
						...
						colorpickerButton.BackgroundColor3 = rc
				...
				function SettingsObj:AddCheckbox(text, default, callback)
					settingsElemCount = settingsElemCount + 1
					local enabled = default or false
					local CheckBoxToggle = New("Frame", { Name = "CheckBoxToggle", Size = UDim2.new(1, 0, 0, 28), BackgroundTransparency = 1, LayoutOrder = settingsElemCount }, SFContainer)
					New("UIAspectRatioConstraint", { AspectRatio = 7.5, AspectType = Enum.AspectType.ScaleWithParentSize }, CheckBoxToggle)
					New("Frame", { Name = "Lines", Position = UDim2.new(0.05, 0, 1, 0), Size = UDim2.new(0.9, 0, 0, 1), BackgroundColor3 = Color3.fromRGB(162, 162, 162), BackgroundTransparency = 0.9, ZIndex = 1000, BorderSizePixel = 0 }, CheckBoxToggle)
					local CheckBox = New("Frame", { Name = "CheckBox", Position = UDim2.new(0.04, 0, 0.3, 0), Size = UDim2.new(0.075, 0, 0.7, 0), BackgroundTransparency = 1, ZIndex = 1001 }, CheckBoxToggle)
					New("UICorner", {}, CheckBox)
					New("UIAspectRatioConstraint", {}, CheckBox)
					local Check = New("ImageLabel", { Name = "Check", Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 1, Image = "rbxassetid://138494545053627", ImageTransparency = enabled and 0.1 or 1, ZIndex = 1001 }, CheckBox)
					New("TextLabel", { Name = "TextToggle", Position = UDim2.new(0.15, 0, 0.3, 0), Size = UDim2.new(0.66, 0, 0.6, 0), BackgroundTransparency = 1, Text = text, TextColor3 = Color3.fromRGB(255, 255, 255), TextScaled = true, Font = Enum.Font.SourceSansSemibold, TextTransparency = 0.1, TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 1001 }, CheckBoxToggle)
					local Btn = New("TextButton", { Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 1, Text = "", ZIndex = 1010 }, CheckBoxToggle)
					local function Set(val)
						enabled = val
			...
			function SectionObj:AddToggle(text, default, callback)
				...
				local enabled = default or false
				local Toggle = New("Frame", { Name = "Toggle", Size = UDim2.new(1, 0, 0.2, 0), BackgroundTransparency = 1, LayoutOrder = elemCount }, Elements)
				New("UIAspectRatioConstraint", { AspectRatio = 9, AspectType = Enum.AspectType.ScaleWithParentSize }, Toggle)
				New("Frame", { Name = "Lines", Position = UDim2.new(0.05, 0, 1, 0), Size = UDim2.new(0.9, 0, 0, 1), BackgroundColor3 = Color3.fromRGB(162, 162, 162), BackgroundTransparency = 0.9, ZIndex = 100, BorderSizePixel = 0 }, Toggle)
				New("TextLabel", { Name = "TextToggle", Position = UDim2.new(0.048, 0, 0.3, 0), Size = UDim2.new(0.66, 0, 0.5, 0), BackgroundTransparency = 1, Text = text, TextColor3 = Color3.fromRGB(255, 255, 255), TextScaled = true, Font = Enum.Font.SourceSansSemibold, TextTransparency = 0.1, TextXAlignment = Enum.TextXAlignment.Left }, Toggle)
				local Effect = New("Frame", { Name = "Effect", Position = UDim2.new(0.8, 0, 0.3, 0), Size = UDim2.new(0.11, 0, 0.56, 0), BackgroundColor3 = enabled and mainColor or Color3.fromRGB(0, 0, 0) }, Toggle)
				New("UICorner", { CornerRadius = UDim.new(0.6, 0) }, Effect)
				New("UIStroke", { Transparency = 0.8, Thickness = 0.8 }, Effect)
				local Icon = New("Frame", { Name = "Icon", Position = enabled and UDim2.new(0.41, 0, 0.045, 0) or UDim2.new(0.08, 0, 0.045, 0), Size = UDim2.new(1, 0, 0.9, 0), BackgroundColor3 = Color3.fromRGB(255,255,255), BackgroundTransparency = enabled and 0 or 0.5 }, Effect)
				New("UICorner", { CornerRadius = UDim.new(1, 0) }, Icon)
				New("UIAspectRatioConstraint", {}, Icon)
				local Btn = New("TextButton", { Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 1, Text = "", ZIndex = 5 }, Toggle)
				local function Set(val)
					enabled = val
					Tween(Effect, { BackgroundColor3 = enabled and mainColor or Color3.fromRGB(0, 0, 0) }, 0.25)
					Tween(Icon, { Position = enabled and UDim2.new(0.41, 0, 0.045, 0) or UDim2.new(0.08, 0, 0.045, 0), BackgroundTransparency = enabled and 0 or 0.5 }, 0.25)
			...
			function SectionObj:AddColorToggle(text, defaultColor, defaultEnabled, callback)
				...
				local T = New("Frame", { Name = "ToggleWithColorPicker", Size = UDim2.new(1, 0, 0.2, 0), BackgroundTransparency = 1, LayoutOrder = elemCount }, Elements)
				New("UIAspectRatioConstraint", { AspectRatio = 9, AspectType = Enum.AspectType.ScaleWithParentSize }, T)
				New("Frame", { Name = "Lines", Position = UDim2.new(0.05, 0, 1, 0), Size = UDim2.new(0.9, 0, 0, 1), BackgroundColor3 = Color3.fromRGB(162, 162, 162), BackgroundTransparency = 0.9, ZIndex = 100, BorderSizePixel = 0 }, T)
				local Effect = New("Frame", { Name = "Effect", Position = UDim2.new(0.8, 0, 0.3, 0), Size = UDim2.new(0.11, 0, 0.56, 0), BackgroundColor3 = enabled and mainColor or Color3.fromRGB(0, 0, 0) }, T)
				New("UICorner", { CornerRadius = UDim.new(0.6, 0) }, Effect)
				New("UIStroke", { Transparency = 0.8, Thickness = 0.8 }, Effect)
				local Icon = New("Frame", { Name = "Icon", Position = enabled and UDim2.new(0.41, 0, 0.045, 0) or UDim2.new(0.08, 0, 0.045, 0), Size = UDim2.new(1, 0, 0.9, 0), BackgroundColor3 = Color3.fromRGB(255, 255, 255) }, Effect)
				New("UICorner", { CornerRadius = UDim.new(1, 0) }, Icon)
				New("UIAspectRatioConstraint", {}, Icon)
				New("TextLabel", { Name = "TextToggle", Position = UDim2.new(0.028, 0, 0.3, 0), Size = UDim2.new(0.66, 0, 0.5, 0), BackgroundTransparency = 1, Text = text, TextColor3 = Color3.fromRGB(255, 255, 255), TextScaled = true, Font = Enum.Font.SourceSansSemibold, TextTransparency = 0.1, TextXAlignment = Enum.TextXAlignment.Left }, T)
				local Btn = New("TextButton", { Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 1, Text = "", ZIndex = 5 }, T)
				local function Set(val)
					enabled = val
					Tween(Effect, { BackgroundColor3 = enabled and mainColor or Color3.fromRGB(0, 0, 0) }, 0.25)
					Tween(Icon, { Position = enabled and UDim2.new(0.41, 0, 0.045, 0) or UDim2.new(0.08, 0, 0.045, 0) }, 0.25)
				...
				function obj:Get() return enabled end
				function obj:AddSettings() return MakeSettings(T, nil, text) end
			...
			function SectionObj:AddToggleColorpicker(text, defaultEnabled, defaultColor, callback, colorCallback)
				...
				New("UIAspectRatioConstraint", { AspectRatio = 9, AspectType = Enum.AspectType.ScaleWithParentSize }, Toggle)
				New("Frame", { Name = "Lines", Position = UDim2.new(0.05, 0, 1, 0), Size = UDim2.new(0.9, 0, 0, 1), BackgroundColor3 = Color3.fromRGB(162, 162, 162), BackgroundTransparency = 0.9, ZIndex = 100, BorderSizePixel = 0 }, Toggle)
				New("TextLabel", { Name = "TextToggle", Position = UDim2.new(0.048, 0, 0.3, 0), Size = UDim2.new(0.6, 0, 0.5, 0), BackgroundTransparency = 1, Text = text, TextColor3 = Color3.fromRGB(255, 255, 255), TextScaled = true, Font = Enum.Font.SourceSansSemibold, TextTransparency = 0.1, TextXAlignment = Enum.TextXAlignment.Left }, Toggle)
				local Effect = New("Frame", { Name = "Effect", Position = UDim2.new(0.8, 0, 0.3, 0), Size = UDim2.new(0.11, 0, 0.56, 0), BackgroundColor3 = enabled and mainColor or Color3.fromRGB(0, 0, 0) }, Toggle)
				New("UICorner", { CornerRadius = UDim.new(0.6, 0) }, Effect)
				New("UIStroke", { Transparency = 0.8, Thickness = 0.8 }, Effect)
				local Icon = New("Frame", { Name = "Icon", Position = enabled and UDim2.new(0.41, 0, 0.045, 0) or UDim2.new(0.08, 0, 0.045, 0), Size = UDim2.new(1, 0, 0.9, 0), BackgroundColor3 = Color3.fromRGB(255, 255, 255), BackgroundTransparency = enabled and 0 or 0.5 }, Effect)
				New("UICorner", { CornerRadius = UDim.new(1, 0) }, Icon)
				New("UIAspectRatioConstraint", {}, Icon)
				local cpBtn = New("ImageButton", { Name = "ColorpickerOpenButton", AnchorPoint = Vector2.new(0.5, 0.5), Position = UDim2.new(0.75, 0, 0.55, 0), Size = UDim2.new(0.068, 0, 0.56, 0), BackgroundColor3 = Color3.fromHSV(color[1], color[2], color[3]), BorderSizePixel = 0, Image = "", ZIndex = 55 }, Toggle)
				New("UICorner", { CornerRadius = UDim.new(0, 6) }, cpBtn)
				local cpFrame = New("Frame", { Name = "Frame", Position = UDim2.new(1.1, 0, 0, 0), Size = UDim2.new(1, 0, 7, 0), BackgroundColor3 = Color3.fromRGB(15,17,26), BorderSizePixel = 0, Visible = false, ZIndex = 350 }, Toggle)
				New("UICorner", { CornerRadius = UDim.new(0, 6) }, cpFrame)
				New("UIStroke", { Color = Color3.fromRGB(255, 255, 255), Transparency = 0.92 }, cpFrame)
				New("UIAspectRatioConstraint", { AspectRatio = 1.1 }, cpFrame)
				local RGB = New("ImageButton", { Name = "RGB", BackgroundTransparency = 1, BorderSizePixel = 0, Position = UDim2.new(0.067, 0, 0.068, 0), Size = UDim2.new(0.74, 0, 0.74, 0), AutoButtonColor = false, Image = "rbxassetid://6523286724", ZIndex = 360 }, cpFrame)
				local RGBCircle = New("ImageLabel", { Name = "RGBCircle", BackgroundTransparency = 1, BorderSizePixel = 0, Size = UDim2.new(0, 14, 0, 14), Image = "rbxassetid://3926309567", ZIndex = 361 }, RGB)
				local Darkness = New("ImageButton", { Name = "Darkness", BackgroundColor3 = Color3.fromRGB(255, 255, 255), BorderSizePixel = 0, Position = UDim2.new(0.832, 0, 0.068, 0), Size = UDim2.new(0.14, 0, 0.74, 0), AutoButtonColor = false, Image = "rbxassetid://156579757", ZIndex = 360 }, cpFrame)
				local DarknessCircle = New("Frame", { Name = "DarknessCircle", AnchorPoint = Vector2.new(0.5, 0.5), BackgroundColor3 = Color3.fromRGB(255, 255, 255), BorderSizePixel = 0, Position = UDim2.new(0.5, 0, 0, 0), Size = UDim2.new(1.4, 0, 0, 5), ZIndex = 361 }, Darkness)
				New("UICorner", { CornerRadius = UDim.new(1, 0) }, DarknessCircle)
				local colorHex = New("TextLabel", { Name = "Hex", BackgroundColor3 = Color3.fromRGB(15,17,26), Position = UDim2.new(0.0717, 0, 0.855, 0), Size = UDim2.new(0.44, 0, 0.09, 0), Font = Enum.Font.ArialBold, Text = "#FFFFFF", TextColor3 = Color3.fromRGB(210, 215, 225), TextScaled = true, ZIndex = 360 }, cpFrame)
				New("UICorner", { CornerRadius = UDim.new(0, 4) }, colorHex)
				local Copy = New("TextButton", { Name = "Copy", BackgroundColor3 = Color3.fromRGB(15,17,26), Position = UDim2.new(0.54, 0, 0.855, 0), Size = UDim2.new(0.39, 0, 0.09, 0), AutoButtonColor = false, Font = Enum.Font.ArialBold, Text = "Copy", TextColor3 = Color3.fromRGB(210, 215, 225), TextScaled = true, ZIndex = 360 }, cpFrame)
				New("UICorner", { CornerRadius = UDim.new(0, 4) }, Copy)
				local Btn = New("TextButton", { Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 1, Text = "", ZIndex = 5 }, Toggle)
				local function to_hex(c) return string.format("#%02X%02X%02X", c.R * 255, c.G * 255, c.B * 255) end
				...
				function obj:Get() return enabled end
				function obj:SetColor(c) local h, s, v = Color3.toHSV(c); setcolor({h, s, v}, true) end
			...
			function SectionObj:AddSlider(text, min, max, default, callback, suffix)
				...
				local initRatio = (value - min) / (max - min)
				local dragging = false
				local Slider = New("Frame", { Name = "Slider", Size = UDim2.new(1, 0, 0.2, 0), BackgroundTransparency = 1, LayoutOrder = elemCount }, Elements)
				New("UIAspectRatioConstraint", { AspectRatio = 9, AspectType = Enum.AspectType.ScaleWithParentSize }, Slider)
				New("Frame", { Name = "Lines", Position = UDim2.new(0.05, 0, 1, 0), Size = UDim2.new(0.9, 0, 0, 1), BackgroundColor3 = Color3.fromRGB(162, 162, 162), BackgroundTransparency = 0.9, ZIndex = 100, BorderSizePixel = 0 }, Slider)
				New("TextLabel", { Name = "TextToggle", Position = UDim2.new(0.048, 0, 0.3, 0), Size = UDim2.new(0.66, 0, 0.5, 0), BackgroundTransparency = 1, Text = text, TextColor3 = Color3.fromRGB(255, 255, 255), TextScaled = true, Font = Enum.Font.SourceSansSemibold, TextTransparency = 0.1, TextXAlignment = Enum.TextXAlignment.Left }, Slider)
				local Effect = New("Frame", { Name = "Effect", Position = UDim2.new(0.498, 0, 0.25, 0), Size = UDim2.new(0.5, 0, 0.7, 0), BackgroundTransparency = 1 }, Slider)
				local SliderValue = New("TextLabel", { Name = "SliderValue", Position = UDim2.new(0.699, 0, 0.1, 0), Size = UDim2.new(0.25, 0, 0.81, 0), BackgroundColor3 = Color3.fromRGB(32, 35, 50), Text = tostring(value) .. suffix, TextColor3 = Color3.fromRGB(255, 255, 255), TextScaled = true, Font = Enum.Font.SourceSansSemibold, TextTransparency = 0.2 }, Effect)
				New("UICorner", { CornerRadius = UDim.new(0, 5) }, SliderValue)
				local Line = New("Frame", { Name = "Line", Position = UDim2.new(0.04, 0, 0.32, 0), Size = UDim2.new(0.6, 0, 0.15, 0), BackgroundColor3 = Color3.fromRGB(33,37,53) }, Effect)
				New("UICorner", { CornerRadius = UDim.new(1, 0) }, Line)
				local InLine = New("Frame", { Name = "InLine", Size = UDim2.new(0, 0, 1, 0), BackgroundColor3 = mainColor, ZIndex = 2 }, Line)
				New("UICorner", { CornerRadius = UDim.new(1, 0) }, InLine)
				local Trigger = New("TextButton", { Name = "Trigger", Position = UDim2.new(0.1, 0, -1.8, 0), Size = UDim2.new(1.2, 0, 4.2, 0), AnchorPoint = Vector2.new(0.8, 0), BackgroundColor3 = Color3.fromRGB(255, 255, 255), Text = "", ZIndex = 3 }, Line)
				New("UICorner", { CornerRadius = UDim.new(1, 0) }, Trigger)
				New("UIAspectRatioConstraint", {}, Trigger)
				task.defer(function()
					InLine.Size = UDim2.fromScale(initRatio, 1)
					Trigger.Position = UDim2.new(initRatio, 0, -1.8, 0)
				end)
				local function update(input)
					local sizeScale = math.clamp((input.Position.X - Line.AbsolutePosition.X) / Line.AbsoluteSize.X, 0, 1)
					value = math.floor(((max - min) * sizeScale) + min)
					Tween(InLine, { Size = UDim2.fromScale(sizeScale, 1) }, 0.1)
					Trigger.Position = UDim2.new(sizeScale, 0, -1.8, 0)
					SliderValue.Text = tostring(value) .. suffix
					if callback then callback(value) end
				end
				Line.InputBegan:Connect(function(input)
				...
				function obj:Set(val)
					...
					local ratio = (value - min) / (max - min)
					InLine.Size = UDim2.fromScale(ratio, 1)
					Trigger.Position = UDim2.new(ratio, 0, -1.8, 0)
					SliderValue.Text = tostring(value) .. suffix
					if callback then callback(value) end
				end
				function obj:Get() return value end
			...
			function SectionObj:AddDropdown(text, options, default, callback)
				...
				local selected = nil
				local dropOpen = false
				local Dropdown = New("Frame", { Name = "Dropdown", Size = UDim2.new(1, 0, 0.2, 0), BackgroundTransparency = 1, LayoutOrder = elemCount, ClipsDescendants = false }, Elements)
				New("UIAspectRatioConstraint", { AspectRatio = 9, AspectType = Enum.AspectType.ScaleWithParentSize }, Dropdown)
				New("Frame", { Name = "Lines", Position = UDim2.new(0.05, 0, 1, 0), Size = UDim2.new(0.9, 0, 0, 1), BackgroundColor3 = Color3.fromRGB(162, 162, 162), BackgroundTransparency = 0.9, ZIndex = 100, BorderSizePixel = 0 }, Dropdown)
				New("TextLabel", { Name = "TextToggle", Position = UDim2.new(0.048, 0, 0.3, 0), Size = UDim2.new(0.6, 0, 0.5, 0), BackgroundTransparency = 1, Text = text, TextColor3 = Color3.fromRGB(255, 255, 255), TextScaled = true, Font = Enum.Font.SourceSansSemibold, TextTransparency = 0.1, TextXAlignment = Enum.TextXAlignment.Left }, Dropdown)
				local TopBar = New("TextButton", { Name = "TopBar", Position = UDim2.new(0.498, 0, 0.25, 0), Size = UDim2.new(0.46, 0, 0.6, 0), BackgroundColor3 = Color3.fromRGB(32, 35, 50), BackgroundTransparency = 0.3, Text = "Select", TextColor3 = Color3.fromRGB(255, 255, 255), TextScaled = true, Font = Enum.Font.SourceSansSemibold, TextTransparency = 0.2, TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 6 }, Dropdown)
				New("UICorner", { CornerRadius = UDim.new(0, 5) }, TopBar)
				local Arrow = New("ImageLabel", { Name = "Arrow", Position = UDim2.new(0.8, 0, 0.17, 0), Size = UDim2.new(0.2, 0, 0.6, 0), BackgroundTransparency = 1, Image = "rbxassetid://10709790948", ImageColor3 = Color3.fromRGB(255, 255, 255), ScaleType = Enum.ScaleType.Fit, Rotation = 0, ZIndex = 7 }, TopBar)
				local DownBar = New("Frame", { Name = "DownBar", Position = UDim2.new(0.56, 0, 0, 0), Size = UDim2.new(0.5, 0, 0, 0), BackgroundColor3 = Color3.fromRGB(16,19,28), Visible = false, ZIndex = 300, ClipsDescendants = true }, Dropdown)
				New("UICorner", {}, DownBar)
				New("UIStroke", { Color = Color3.fromRGB(255, 255, 255), Transparency = 0.98, Thickness = 5 }, DownBar)
				local Scrolls = New("ScrollingFrame", { Name = "Scrolls", Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 1, BorderSizePixel = 0, CanvasSize = UDim2.new(0, 0, 0, #options * 24), ScrollBarThickness = 1, ZIndex = 301 }, DownBar)
				...
				for _, opt in ipairs(options) do
					local Buttons = New("TextButton", { Name = "Buttons", Size = UDim2.new(1, 0, 0, 16), BackgroundColor3 = Color3.fromRGB(32, 35, 50), BackgroundTransparency = 1, Text = opt, TextColor3 = Color3.fromRGB(255, 255, 255), TextScaled = true, Font = Enum.Font.SourceSansSemibold, TextTransparency = 0.2, TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 302 }, Scrolls)
					New("UICorner", { CornerRadius = UDim.new(0, 5) }, Buttons)
					Buttons.MouseEnter:Connect(function() Tween(Buttons, { BackgroundTransparency = 0.6 }, 0.1) end)
					Buttons.MouseLeave:Connect(function() Tween(Buttons, { BackgroundTransparency = selected == opt and 0.5 or 1 }, 0.1) end)
			...
			function SectionObj:AddDropdownColorpicker(text, options, defaultColor, callback, colorCallback)
				...
				local Dropdown = New("Frame", { Name = "Dropdown", Size = UDim2.new(1, 0, 0.2, 0), BackgroundTransparency = 1, LayoutOrder = elemCount, ClipsDescendants = false }, Elements)
				New("UIAspectRatioConstraint", { AspectRatio = 9, AspectType = Enum.AspectType.ScaleWithParentSize }, Dropdown)
				New("Frame", { Name = "Lines", Position = UDim2.new(0.05, 0, 1, 0), Size = UDim2.new(0.9, 0, 0, 1), BackgroundColor3 = Color3.fromRGB(162, 162, 162), BackgroundTransparency = 0.9, ZIndex = 100 }, Dropdown)
				New("TextLabel", { Name = "TextDropdown", Position = UDim2.new(0.048, 0, 0.3, 0), Size = UDim2.new(0.6, 0, 0.5, 0), BackgroundTransparency = 1, Text = text, TextColor3 = Color3.fromRGB(255, 255, 255), TextScaled = true, Font = Enum.Font.SourceSansSemibold, TextTransparency = 0.1, TextXAlignment = Enum.TextXAlignment.Left }, Dropdown)
				local cpBtn = New("ImageButton", { Name = "ColorpickerOpenButton", AnchorPoint = Vector2.new(0.5, 0.5), Position = UDim2.new(0.44, 0, 0.55, 0), Size = UDim2.new(0.068, 0, 0.56, 0), BackgroundColor3 = Color3.fromHSV(hue, sat, val), BorderSizePixel = 0, Image = "", ZIndex = 55 }, Dropdown)
				New("UICorner", { CornerRadius = UDim.new(0, 6) }, cpBtn)
				local TopBar = New("TextButton", { Name = "TopBar", Position = UDim2.new(0.498, 0, 0.25, 0), Size = UDim2.new(0.46, 0, 0.6, 0), BackgroundColor3 = Color3.fromRGB(32, 35, 50), BackgroundTransparency = 0.3, Text = "Select", TextColor3 = Color3.fromRGB(255, 255, 255), TextScaled = true, Font = Enum.Font.SourceSansSemibold, TextTransparency = 0.2, TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 6 }, Dropdown)
				New("UICorner", { CornerRadius = UDim.new(0, 5) }, TopBar)
				local Arrow = New("ImageLabel", { Name = "Arrow", Position = UDim2.new(0.8, 0, 0.17, 1), Size = UDim2.new(0.2, 0, 0.6, 0), BackgroundTransparency = 1, Image = "rbxassetid://10709790948", ImageColor3 = Color3.fromRGB(255, 255, 255), ScaleType = Enum.ScaleType.Fit, Rotation = 0, ZIndex = 7 }, TopBar)
				local DownBar = New("Frame", { Name = "DownBar", Position = UDim2.new(0.56, 0, 0, 0), Size = UDim2.new(0.5, 0, 0, 0), BackgroundColor3 = Color3.fromRGB(16,19,28), ClipsDescendants = true, Visible = false, ZIndex = 300 }, Dropdown)
				New("UICorner", {}, DownBar)
				New("UIStroke", { Color = Color3.fromRGB(255, 255, 255), Transparency = 0.98, Thickness = 5 }, DownBar)
				local Scrolls = New("ScrollingFrame", { Name = "Scrolls", Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 1, BorderSizePixel = 0, CanvasSize = UDim2.new(0, 0, 0, #options * 24), ScrollBarThickness = 1, ZIndex = 301 }, DownBar)
				New("UIListLayout", { Padding = UDim.new(0, 2), SortOrder = Enum.SortOrder.LayoutOrder }, Scrolls)
				...
				for _, opt in ipairs(options) do
					local Btn = New("TextButton", { Name = "Buttons", Size = UDim2.new(1, 0, 0, 16), BackgroundColor3 = Color3.fromRGB(32, 35, 50), BackgroundTransparency = 1, Text = opt, TextColor3 = Color3.fromRGB(255, 255, 255), TextScaled = true, Font = Enum.Font.SourceSansSemibold, TextTransparency = 0.2, TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 302 }, Scrolls)
					New("UICorner", { CornerRadius = UDim.new(0, 5) }, Btn)
					Btn.MouseEnter:Connect(function() Tween(Btn, { BackgroundTransparency = 0.6 }, 0.1) end)
					Btn.MouseLeave:Connect(function() Tween(Btn, { BackgroundTransparency = selected == opt and 0.5 or 1 }, 0.1) end)
					Btn.MouseButton1Click:Connect(function()
						selected = opt
						TopBar.Text = opt
						closeDropdown()
				...
				local cpFrame = New("Frame", { Name = "colorpickerFrame", Position = UDim2.new(1.1, 0, 0, 0), Size = UDim2.new(1, 0, 7, 0), BackgroundColor3 = Color3.fromRGB(15,17,26), BorderSizePixel = 0, Visible = false, ZIndex = 350, ClipsDescendants = false }, Dropdown)
				New("UICorner", { CornerRadius = UDim.new(0, 6) }, cpFrame)
				New("UIStroke", { Color = Color3.fromRGB(255, 255, 255), Transparency = 0.92, Thickness = 1 }, cpFrame)
				New("UIAspectRatioConstraint", { AspectRatio = 1.1 }, cpFrame)
				local RGB = New("ImageButton", { Name = "RGB", BackgroundTransparency = 1, BorderSizePixel = 0, Position = UDim2.new(0.067, 0, 0.068, 0), Size = UDim2.new(0.74, 0, 0.74, 0), AutoButtonColor = false, Image = "rbxassetid://6523286724", ZIndex = 360 }, cpFrame)
				local RGBCircle = New("ImageLabel", { Name = "RGBCircle", BackgroundTransparency = 1, BorderSizePixel = 0, Size = UDim2.new(0, 14, 0, 14), Image = "rbxassetid://3926309567", ImageRectOffset = Vector2.new(628, 420), ImageRectSize = Vector2.new(48, 48), ZIndex = 361 }, RGB)
				local Darkness = New("ImageButton", { Name = "Darkness", BackgroundColor3 = Color3.fromRGB(255, 255, 255), BorderSizePixel = 0, Position = UDim2.new(0.832, 0, 0.068, 0), Size = UDim2.new(0.14, 0, 0.74, 0), AutoButtonColor = false, Image = "rbxassetid://156579757", ZIndex = 360 }, cpFrame)
				local DarknessCircle = New("Frame", { Name = "DarknessCircle", AnchorPoint = Vector2.new(0.5, 0.5), BackgroundColor3 = Color3.fromRGB(255, 255, 255), BorderSizePixel = 0, Position = UDim2.new(0.5, 0, 0, 0), Size = UDim2.new(1.4, 0, 0, 5), ZIndex = 361 }, Darkness)
				New("UICorner", { CornerRadius = UDim.new(1, 0) }, DarknessCircle)
				local colorHex = New("TextLabel", { Name = "colorHex", BackgroundColor3 = Color3.fromRGB(15,17,26), Position = UDim2.new(0.0717, 0, 0.855, 0), Size = UDim2.new(0.44, 0, 0.09, 0), Font = Enum.Font.ArialBold, Text = "#FFFFFF", TextColor3 = Color3.fromRGB(210, 215, 225), TextScaled = true, ZIndex = 360 }, cpFrame)
				New("UICorner", { CornerRadius = UDim.new(0, 4) }, colorHex)
				local Copy = New("TextButton", { Name = "Copy", BackgroundColor3 = Color3.fromRGB(15,17,26), Position = UDim2.new(0.54, 0, 0.855, 0), Size = UDim2.new(0.39, 0, 0.09, 0), AutoButtonColor = false, Font = Enum.Font.ArialBold, Text = "Copy", TextColor3 = Color3.fromRGB(210, 215, 225), TextScaled = true, ZIndex = 360 }, cpFrame)
				New("UICorner", { CornerRadius = UDim.new(0, 4) }, Copy)
				local function to_hex(c) return string.format("#%02X%02X%02X", c.R * 255, c.G * 255, c.B * 255) end
			...
			function SectionObj:AddColorpicker(text, defaultColor, callback)
				...
				local WheelDown = false
				local SlideDown = false
				local Colorpicker = New("Frame", { Name = "Colorpicker", Size = UDim2.new(1, 0, 0.2, 0), BackgroundTransparency = 1, LayoutOrder = elemCount, ClipsDescendants = false, ZIndex = 2 }, Elements)
				New("UIAspectRatioConstraint", { AspectRatio = 9, AspectType = Enum.AspectType.ScaleWithParentSize }, Colorpicker)
				New("Frame", { Name = "Lines", Position = UDim2.new(0.05, 0, 1, 0), Size = UDim2.new(0.89, 0, 0, 1), BackgroundColor3 = Color3.fromRGB(162, 162, 162), BackgroundTransparency = 0.9, BorderSizePixel = 0, ZIndex = 3 }, Colorpicker)
				New("TextLabel", { Position = UDim2.new(0.048, 0, 0.3, 0), Size = UDim2.new(0.6, 0, 0.5, 0), BackgroundTransparency = 1, Text = text, TextColor3 = Color3.fromRGB(255, 255, 255), TextScaled = true, Font = Enum.Font.SourceSansSemibold, TextTransparency = 0.1, TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 10 }, Colorpicker)
				local cpBtn = New("ImageButton", { Name = "colorpickerButton", AnchorPoint = Vector2.new(0.5, 0.5), Position = UDim2.new(0.875, 0, 0.57, 0), Size = UDim2.new(0.068, 0, 0.56, 0), BackgroundColor3 = defaultColor or Color3.fromRGB(255, 255, 255), BorderSizePixel = 0, Image = "", ZIndex = 30 }, Colorpicker)
				New("UICorner", { CornerRadius = UDim.new(0, 6) }, cpBtn)
				local cpFrame = New("Frame", { Name = "colorpickerFrame", Position = UDim2.new(1.1, 0, 0, 0), Size = UDim2.new(1, 0, 7, 0), BackgroundColor3 = Color3.fromRGB(15,17,26), BorderSizePixel = 0, Visible = false, ZIndex = 350 }, Colorpicker)
				New("UICorner", { CornerRadius = UDim.new(0, 6) }, cpFrame)
				New("UIStroke", { Color = Color3.fromRGB(255, 255, 255), Transparency = 0.92, Thickness = 1 }, cpFrame)
				New("UIAspectRatioConstraint", { AspectRatio = 1.1 }, cpFrame)
				local RGB = New("ImageButton", { Name = "RGB", BackgroundTransparency = 1, BorderSizePixel = 0, Position = UDim2.new(0.067, 0, 0.068, 0), Size = UDim2.new(0.74, 0, 0.74, 0), AutoButtonColor = false, Image = "rbxassetid://6523286724", ZIndex = 360 }, cpFrame)
				...
				local function UpdateSlide()
					...
					local rc = Color3.fromHSV(color[1], color[2], color[3])
					DarknessCircle.BackgroundColor3 = rc; DarknessCircle.Position = UDim2.new(0.5, 0, y, -cy)
					if callback then callback(rc) end
					update()
				end
				local function UpdateRing()
					local m = ml()
			...
			function SectionObj:AddAccordion(text)
				...
				TextArrow.Position = UDim2.new(0.87, 0, 0.1, 0)
				...
				TextArrow.ImageTransparency = 0.1
				...
				Text.Name = "Text"
				Text.Position = UDim2.new(0.048, 0, 0.3, 0)
				Text.Size = UDim2.new(0.66, 0, 0.5, 0)
				...
				do local c = Instance.new('UICorner'); c.Parent = Section2Frame end
				do local s = Instance.new('UIStroke'); s.Color = Color3.fromRGB(255, 255, 255); s.Transparency = 0.9; s.Parent = Section2Frame end

				local LabelContainer = Instance.new('TextLabel')
				LabelContainer.Name = "LabelContainer"
				LabelContainer.Position = UDim2.new(0.05,0,0,0)
				LabelContainer.Size = UDim2.new(1,0,0.3,0)
				...
				do
					local a = Instance.new('UIAspectRatioConstraint', LabelContainer)
					a.AspectRatio = 14
					a.AspectType = Enum.AspectType.ScaleWithParentSize
				end
				...
				Container.Position = UDim2.new(0.05, 0, 1, 0)
				Container.Size = UDim2.new(0.9, 0, 0.97, 20)
				...
				New('UIStroke', { Color = Color3.fromRGB(255,255,255), Transparency = 0.9, ApplyStrokeMode = Enum.ApplyStrokeMode.Border }, Container)
				local UIListLayout = Instance.new('UIListLayout')
				UIListLayout.Padding = UDim.new(0, 3)
				UIListLayout.SortOrder = Enum.SortOrder.LayoutOrder
				UIListLayout.Parent = Container

				New("Frame", { Name = "Lines", Position = UDim2.new(0.05, 0, 1, 0), Size = UDim2.new(0.9, 0, 0, 1), BackgroundColor3 = Color3.fromRGB(162, 162, 162), BackgroundTransparency = 0.9, ZIndex = 100, BorderSizePixel = 0 }, DropdownSection)
				...
				local function closeAccordion()
					...
					Section2Frame.Size = UDim2.new(0.898, 0, 0, Section2Frame.AbsoluteSize.Y)
					Tween(Section2Frame, { Size = UDim2.new(0.898, 0, 0, 0) }, 0.2, Enum.EasingStyle.Quad, Enum.EasingDirection.In)
				...
				Open.MouseButton1Click:Connect(function()
					if accordionOpen then
						...
						Section2Frame.BackgroundTransparency = 1
						Section2Frame.Size = UDim2.new(0.898, 0, 0, 0)
						task.defer(function()
							Tween(Section2Frame, { BackgroundTransparency = 0.02 }, 0.3, Enum.EasingStyle.Back, Enum.EasingDirection.Out)
						end)
					end
				end)
				...
				function AccordionObj:AddToggle(text2, default, callback)
					...
					local enabled = default or false
					local Toggle = New("Frame", { Name = "Toggle", Size = UDim2.new(1, 0, 0, 28), BackgroundTransparency = 1, LayoutOrder = accordionElemCount }, Container)
					New("UIAspectRatioConstraint", { AspectRatio = 7.5, AspectType = Enum.AspectType.ScaleWithParentSize }, Toggle)
					New("Frame", { Name = "Lines", Position = UDim2.new(0.05, 0, 1, 0), Size = UDim2.new(0.9, 0, 0, 1), BackgroundColor3 = Color3.fromRGB(162, 162, 162), BackgroundTransparency = 0.9, ZIndex = 100, BorderSizePixel = 0 }, Toggle)
					New("TextLabel", { Name = "TextToggle", Position = UDim2.new(0.048, 0, 0.3, 0), Size = UDim2.new(0.66, 0, 0.6, 0), BackgroundTransparency = 1, Text = text2, TextColor3 = Color3.fromRGB(255, 255, 255), TextScaled = true, Font = Enum.Font.SourceSansSemibold, TextTransparency = 0.1, TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 101 }, Toggle)
					local Effect = New("Frame", { Name = "Effect", Position = UDim2.new(0.8, 0, 0.3, 0), Size = UDim2.new(0.11, 0, 0.56, 0), BackgroundColor3 = enabled and mainColor or Color3.fromRGB(0, 0, 0), ZIndex = 101 }, Toggle)
					New("UICorner", { CornerRadius = UDim.new(0.6, 0) }, Effect)
					New("UIStroke", { Transparency = 0.8, Thickness = 0.8 }, Effect)
					local Icon = New("Frame", { Name = "Icon", Position = enabled and UDim2.new(0.41, 0, 0.045, 0) or UDim2.new(0.08, 0, 0.045, 0), Size = UDim2.new(1, 0, 0.9, 0), BackgroundColor3 = Color3.fromRGB(255, 255, 255), BackgroundTransparency = enabled and 0 or 0.5, ZIndex = 102 }, Effect)
					New("UICorner", { CornerRadius = UDim.new(1, 0) }, Icon)
					New("UIAspectRatioConstraint", {}, Icon)
					local Btn = New("TextButton", { Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 1, Text = "", ZIndex = 110 }, Toggle)
					local function Set(val)
						enabled = val
						Tween(Effect, { BackgroundColor3 = enabled and mainColor or Color3.fromRGB(0, 0, 0) }, 0.25)
						Tween(Icon, { Position = enabled and UDim2.new(0.41, 0, 0.045, 0) or UDim2.new(0.08, 0, 0.045, 0), BackgroundTransparency = enabled and 0 or 0.5 }, 0.25)
				...
				function AccordionObj:AddCheckbox(text2, default, callback)
					accordionElemCount = accordionElemCount + 1
					local enabled = default or false
					local CheckBoxToggle = New("Frame", { Name = "CheckBoxToggle", Size = UDim2.new(1, 0, 0, 28), BackgroundTransparency = 1, LayoutOrder = accordionElemCount }, Container)
					New("UIAspectRatioConstraint", { AspectRatio = 7.5, AspectType = Enum.AspectType.ScaleWithParentSize }, CheckBoxToggle)
					New("Frame", { Name = "Lines", Position = UDim2.new(0.05, 0, 1, 0), Size = UDim2.new(0.9, 0, 0, 1), BackgroundColor3 = Color3.fromRGB(162, 162, 162), BackgroundTransparency = 0.9, ZIndex = 100, BorderSizePixel = 0 }, CheckBoxToggle)
					local CheckBox = New("Frame", { Name = "CheckBox", Position = UDim2.new(0.04, 0, 0.3, 0), Size = UDim2.new(0.075, 0, 0.7, 0), BackgroundTransparency = 1, ZIndex = 101 }, CheckBoxToggle)
					New("UICorner", {}, CheckBox)
					New("UIAspectRatioConstraint", {}, CheckBox)
					local Check = New("ImageLabel", { Name = "Check", Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 1, Image = "rbxassetid://138494545053627", ImageTransparency = enabled and 0.1 or 1, ZIndex = 101 }, CheckBox)
					New("TextLabel", { Name = "TextToggle", Position = UDim2.new(0.15, 0, 0.3, 0), Size = UDim2.new(0.66, 0, 0.6, 0), BackgroundTransparency = 1, Text = text2, TextColor3 = Color3.fromRGB(255, 255, 255), TextScaled = true, Font = Enum.Font.SourceSansSemibold, TextTransparency = 0.1, TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 101 }, CheckBoxToggle)
					local Btn = New("TextButton", { Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 1, Text = "", ZIndex = 110 }, CheckBoxToggle)
					local function Set(val)
						enabled = val
						Tween(Check, { ImageTransparency = enabled and 0.1 or 1 }, 0.2)
						if callback then callback(enabled) end
				...
				function AccordionObj:AddDropdown(text2, options, default, callback)
					...
					local dropOpen = false
					local selectedBtns = {}
					local Selection = New("Frame", { Name = "Selection", Size = UDim2.new(1, 0, 0, 28), BackgroundTransparency = 1, LayoutOrder = accordionElemCount, ClipsDescendants = false, ZIndex = 100 }, Container)
					New("UIAspectRatioConstraint", { AspectRatio = 7.5, AspectType = Enum.AspectType.ScaleWithParentSize }, Selection)
					New("Frame", { Name = "Lines", Position = UDim2.new(0.05, 0, 1, 0), Size = UDim2.new(0.9, 0, 0, 1), BackgroundColor3 = Color3.fromRGB(162, 162, 162), BackgroundTransparency = 0.9, BorderSizePixel = 0, ZIndex = 100 }, Selection)
					New("TextLabel", { Name = "TextToggle", Position = UDim2.new(0.048, 0, 0.3, 0), Size = UDim2.new(0.6, 0, 0.6, 0), BackgroundTransparency = 1, Text = text2, TextColor3 = Color3.fromRGB(255, 255, 255), TextScaled = true, Font = Enum.Font.SourceSansSemibold, TextTransparency = 0.1, TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 101 }, Selection)
					local SelArrow = New("ImageLabel", { Name = "TextArrow", Position = UDim2.new(0.87, 0, 0.15, 0), Size = UDim2.new(0.1, 0, 0.8, 0), BackgroundTransparency = 1, Image = "rbxassetid://10709790948", ImageColor3 = Color3.fromRGB(255, 255, 255), ImageTransparency = 0.1, ScaleType = Enum.ScaleType.Fit, Rotation = -90, ZIndex = 1000 }, Selection)
					local TextDefault = New("TextLabel", { Name = "TextDefault", Position = UDim2.new(0.64, 0, 0.25, 0), Size = UDim2.new(0.23, 0, 0.6, 0), BackgroundTransparency = 1, Text = options[1] or "Auto", TextColor3 = Color3.fromRGB(255, 255, 255), TextScaled = true, Font = Enum.Font.SourceSansSemibold, TextTransparency = 0.5, ZIndex = 101, TextXAlignment = Enum.TextXAlignment.Right }, Selection)
					local DropPopup = New("Frame", { Name = "Dropdown", Position = UDim2.new(1, 0, 0, 0), Size = UDim2.new(0.69, 0, 0, 100), BackgroundColor3 = Color3.fromRGB(16,19,28), BackgroundTransparency = 1, BorderSizePixel = 0, Visible = false, ZIndex = 1000 }, Selection)
					New("UICorner", { CornerRadius = UDim.new(0, 5) }, DropPopup)
					New("UIStroke", { Color = Color3.fromRGB(28,32,48), Transparency = 0.8, ApplyStrokeMode = Enum.ApplyStrokeMode.Border }, DropPopup)
					local DropContainer = New("Frame", { Name = "Container", Position = UDim2.new(0.05, 0, 0.1, 0), Size = UDim2.new(0.898, 0, 0.8, 0), BackgroundTransparency = 1, ZIndex = 1000, AutomaticSize = Enum.AutomaticSize.Y }, DropPopup)
					...
					for i, opt in ipairs(options) do
						...
						local RowBtn = New("TextButton", { Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 1, Text = "", ZIndex = 1005 }, BtnRow)
						RowBtn.MouseButton1Click:Connect(function()
							selected = opt; TextDefault.Text = opt; updateCheckmarks(); closeDropdown2()
							if callback then callback(opt) end
						end)
						SelBtn.MouseButton1Click:Connect(function()
							selected = opt; TextDefault.Text = opt; updateCheckmarks(); closeDropdown2()
					...
					local OpenBtn = New("TextButton", { Name = "OpenBtn", Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 1, Text = "", ZIndex = 500 }, Selection)
					OpenBtn.MouseButton1Click:Connect(function()
				...
				function AccordionObj:AddSlider(text2, min, max, default, callback, suffix)
					...
					local initRatio = (value - min) / (max - min)
					local dragging = false
					local Slider = New("Frame", { Name = "Slider", Size = UDim2.new(1, 0, 0.2, 0), BackgroundTransparency = 1, LayoutOrder = accordionElemCount, ZIndex = 100 }, Container)
					New("UIAspectRatioConstraint", { AspectRatio = 7.5, AspectType = Enum.AspectType.ScaleWithParentSize }, Slider)
					New("Frame", { Name = "Lines", Position = UDim2.new(0.05, 0, 1, 0), Size = UDim2.new(0.9, 0, 0, 1), BackgroundColor3 = Color3.fromRGB(162, 162, 162), BackgroundTransparency = 0.9, ZIndex = 1002, BorderSizePixel = 0 }, Slider)
					New("TextLabel", { Name = "TextToggle", Position = UDim2.new(0.048, 0, 0.3, 0), Size = UDim2.new(0.66, 0, 0.6, 0), BackgroundTransparency = 1, Text = text2, TextColor3 = Color3.fromRGB(255, 255, 255), TextScaled = true, Font = Enum.Font.SourceSansSemibold, TextTransparency = 0.1, TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 101 }, Slider)
					local Effect = New("Frame", { Name = "Effect", Position = UDim2.new(0.498, 0, 0.25, 0), Size = UDim2.new(0.5, 0, 0.7, 0), BackgroundTransparency = 1, ZIndex = 101 }, Slider)
					local SliderValue = New("TextLabel", { Name = "SliderValue", Position = UDim2.new(0.699, 0, 0.1, 0), Size = UDim2.new(0.25, 0, 0.81, 0), BackgroundColor3 = Color3.fromRGB(33,37,53), Text = tostring(value) .. suffix, TextColor3 = Color3.fromRGB(255, 255, 255), TextScaled = true, Font = Enum.Font.SourceSansSemibold, TextTransparency = 0.2, ZIndex = 102 }, Effect)
					New("UICorner", { CornerRadius = UDim.new(0, 5) }, SliderValue)
					local Line = New("Frame", { Name = "Line", Position = UDim2.new(0.04, 0, 0.32, 0), Size = UDim2.new(0.6, 0, 0.24, 0), BackgroundColor3 = Color3.fromRGB(33,37,53), ZIndex = 101 }, Effect)
					New("UICorner", { CornerRadius = UDim.new(1, 0) }, Line)
					local InLine = New("Frame", { Name = "InLine", Size = UDim2.new(0, 0, 1, 0), BackgroundColor3 = mainColor, ZIndex = 102 }, Line)
					New("UICorner", { CornerRadius = UDim.new(1, 0) }, InLine)
					local Trigger = New("TextButton", { Name = "Trigger", Position = UDim2.new(0.1, 0, -1.8, 0), Size = UDim2.new(1.2, 0, 4.2, 0), AnchorPoint = Vector2.new(0.8, 0), BackgroundTransparency = 1, Text = "", ZIndex = 103 }, Line)
					New("UICorner", { CornerRadius = UDim.new(1, 0) }, Trigger)
					New("UIAspectRatioConstraint", {}, Trigger)
					task.defer(function()
						InLine.Size = UDim2.fromScale(initRatio, 1)
				...
				function AccordionObj:AddColorpicker(text2, defaultColor, callback)
					...
					local WheelDown = false
					local SlideDown = false
					local Colorpicker = New("Frame", { Name = "Colorpicker", Size = UDim2.new(1, 0, 0, 28), BackgroundTransparency = 1, LayoutOrder = accordionElemCount, ClipsDescendants = false, ZIndex = 100 }, Container)
					New("UIAspectRatioConstraint", { AspectRatio = 7.5, AspectType = Enum.AspectType.ScaleWithParentSize }, Colorpicker)
					New("Frame", { Name = "Lines", Position = UDim2.new(0.05, 0, 1, 0), Size = UDim2.new(0.89, 0, 0, 1), BackgroundColor3 = Color3.fromRGB(162, 162, 162), BackgroundTransparency = 0.9, BorderSizePixel = 0, ZIndex = 101 }, Colorpicker)
					New("TextLabel", { Position = UDim2.new(0.048, 0, 0.3, 0), Size = UDim2.new(0.66, 0, 0.6, 0), BackgroundTransparency = 1, Text = text2, TextColor3 = Color3.fromRGB(255, 255, 255), TextScaled = true, Font = Enum.Font.SourceSansBold, TextTransparency = 0.1, TextXAlignment = Enum.TextXAlignment.Left, ZIndex = 101 }, Colorpicker)
					local cpBtn = New("ImageButton", { Name = "colorpickerButton", AnchorPoint = Vector2.new(0.5, 0.5), Position = UDim2.new(0.875, 0, 0.57, 0), Size = UDim2.new(0.08, 0, 0.5, 0), BackgroundColor3 = defaultColor or Color3.fromRGB(255, 255, 255), BorderSizePixel = 0, Image = "", ZIndex = 110 }, Colorpicker)
					New("UICorner", { CornerRadius = UDim.new(0, 6) }, cpBtn)
					local cpFrame = New("Frame", { Name = "colorpickerFrame", Position = UDim2.new(1.1, 0, 0, 0), Size = UDim2.new(1, 0, 7, 0), BackgroundColor3 = Color3.fromRGB(15,17,26), BorderSizePixel = 0, Visible = false, ZIndex = 350 }, Colorpicker)
					New("UICorner", { CornerRadius = UDim.new(0, 6) }, cpFrame)
					New("UIStroke", { Color = Color3.fromRGB(255, 255, 255), Transparency = 0.92, Thickness = 1 }, cpFrame)
					...
					local DarknessCircle = New("Frame", { Name = "DarknessCircle", AnchorPoint = Vector2.new(0.5, 0.5), BackgroundColor3 = Color3.fromRGB(255, 255, 255), BorderSizePixel = 0, Position = UDim2.new(0.5, 0, 0, 0), Size = UDim2.new(1.4, 0, 0, 5), ZIndex = 361 }, Darkness)
					New("UICorner", { CornerRadius = UDim.new(1, 0) }, DarknessCircle)
					local colorHex = New("TextLabel", { BackgroundColor3 = Color3.fromRGB(15,17,26), Position = UDim2.new(0.0717, 0, 0.855, 0), Size = UDim2.new(0.44, 0, 0.09, 0), Font = Enum.Font.ArialBold, Text = "#FFFFFF", TextColor3 = Color3.fromRGB(210, 215, 225), TextScaled = true, ZIndex = 360 }, cpFrame)
					New("UICorner", { CornerRadius = UDim.new(0, 4) }, colorHex)
					local Copy = New("TextButton", { BackgroundColor3 = Color3.fromRGB(15,17,26), Position = UDim2.new(0.54, 0, 0.855, 0), Size = UDim2.new(0.39, 0, 0.09, 0), AutoButtonColor = false, Font = Enum.Font.ArialBold, Text = "Copy", TextColor3 = Color3.fromRGB(210, 215, 225), TextScaled = true, ZIndex = 360 }, cpFrame)
					New("UICorner", { CornerRadius = UDim.new(0, 4) }, Copy)
					local function to_hex(c) return string.format("#%02X%02X%02X", c.R * 255, c.G * 255, c.B * 255) end
					local function update()
						local c = Color3.fromHSV(color[1], color[2], color[3])
			...
			ShowContent = function()
				...
				if activeSubTabIndex then
					...
					if st then
						...
						Tween(st.left,  { Position = UDim2.new(0, 0, 0, 0) }, 0.25)
						Tween(st.right, { Position = UDim2.new(0, 0, 0, 0) }, 0.25)
					end
		...
		function TabObj:AddSubTab(name, icon)
			...
			New("UIAspectRatioConstraint", { AspectRatio = 4.6, AspectType = Enum.AspectType.ScaleWithParentSize }, STRow)
			...
			local function ActivateSubTab()
				...
				Tween(stLeft,  { Position = UDim2.new(0, 0, 0, 0) }, 0.25)
				Tween(stRight, { Position = UDim2.new(0, 0, 0, 0) }, 0.25)
	...
	-- DELETION CLEANUP
	-- ============================================================
	function WindowObj:Delete()
		_alive = false
		table.clear(tabs)
		table.clear(_openPopups)
		if AcrylicBlur and AcrylicBlur.Destroy then AcrylicBlur.Destroy() end
		NeverloseCS2:Destroy()
	end
...
-- LIBRARY PUBLIC API
-- ============================================================
function Library:ClearRegistry()
	table.clear(_registeredElements)
end
